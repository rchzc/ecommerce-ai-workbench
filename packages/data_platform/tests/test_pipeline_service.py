...
class TestPipelineRun:
    def test_full_run_ok(self, tmp_path, isolated_paths):
    ...
    def test_idempotent_skip(self, tmp_path, isolated_paths):
        service = build_pipeline(tmp_path)
    ...
    def test_state_restored_on_new_instance(self, tmp_path, isolated_paths):
        run(build_pipeline(tmp_path).run(force=True))
        # 新实例从状态文件恢复，同样幂等跳过 —— 重启服务不会重复同步
        result = run(build_pipeline(tmp_path).run())
        assert result.get("skipped") is True

    def test_refresh_is_deterministic(self, tmp_path, isolated_paths):
    ...
    def test_feishu_failure_recorded_not_raised(self, tmp_path, isolated_paths):
    ...
    def test_refresh_disabled(self, tmp_path, isolated_paths):
    ...
    def test_force_recovers_after_failure(self, tmp_path, isolated_paths):
...
class TestStateFile:
    def test_state_file_from_config(self, isolated_paths):
        """状态文件位置只能来自 .config —— 以前是各模块按目录深度自己算的。"""
        assert os.path.basename(ps.STATE_PATH) == "pipeline_state.json"
        assert os.path.dirname(ps.STATE_PATH) == str(isolated_paths)

    def test_corrupt_state_file_does_not_crash(self, tmp_path, monkeypatch):
        """状态文件被写坏时应当作"没有历史状态"继续跑，不能启动就崩。"""
        monkeypatch.setattr(ps, "STATE_PATH", str(tmp_path / "bad.json"))
        (tmp_path / "bad.json").write_text("{ not json", encoding="utf-8")
        assert build_pipeline(tmp_path).last_run == {}
